...
var mongoose = require('mongoose'),
	...
	async = require('async'),
	geocoder = require('node-geocoder').getGeocoder("google", "http", { apiKey: config.keys.google.simpleApiKey }),
...
module.exports = {
			...
			async.each(response.groups, function(rawChapter, groupCallback) {
				...
				Chapter.findOne({ _id: rawChapter.gplus_id }, function (err, chapter) {
					...
					var processChapter = function(chapter) {
						...
						chapter.country = rawChapter.country;
					...
					plusQueue.push("https://www.googleapis.com/plus/v1/people/"+chapter._id+"?fields=urls&key="+config.keys.google.simpleApiKey, function(err, res) {
						...
						async.each(lookup, function(url, callback) {
							...
							if((rawChapter.geo && rawChapter.geo.lng) || (!isNew && chapter.geo )) {
								chapter.geo = {
	...
	fetchTags: function(callback) {
		if(!callback) return;
		request.get("https://developers.google.com/tags/gettags").retry(2).end(function(err, res) {
			if(err) {
	...
	fetchTaggedEvents: function(tag, callback) {
		...
		request.get("https://google-developers.appspot.com/events/event-markers.public?tag="+tag+"&start="+firstDayOfMonth+"&end="+lastDayOfMonth+"&_="+now).retry(2).end(function(err, res) {
			if(err) {
				return callback(err, null);
			}
	...
	fetchEventsForChapter: function(month, chapterId, callback) {

		var lastDayOfMonth = moment().month(month).add('months', 1).date(1).subtract('days', 1).seconds(0).minutes(0).hours(0).unix();
		var firstDayOfMonth = moment().month(month).date(1).seconds(0).minutes(0).hours(0).unix();
		var now = moment().unix();
		request.get("https://developers.google.com/events/feed/json?group="+chapterId+"&start="+firstDayOfMonth+"&end="+lastDayOfMonth).retry(2).end(function(error, result) {
			if(error) {
				if(callback)
					callback(error);
				return;
