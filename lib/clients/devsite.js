...
module.exports = {
	...
	fetchTaggedEvents: function(tag, callback) {
		...
		request.get("https://google-developers.appspot.com/events/event-markers.public?tag="+tag+"&start="+firstDayOfMonth+"&end="+lastDayOfMonth+"&_="+now).retry(5).end(function(err, res) {
			if(err) {
				return callback(err, null);
			}
