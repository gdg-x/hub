...
module.exports = {
	...
	recordMonthlyMetric: function(subject, subjectType, metricType, value, cb) {
		...
		MonthlyMetric.findOne({ year: moment().year(), subject: subject, subjectType: subjectType, type: metricType }, function(err, metric) {

			var saveMetric = function() {
				var set = {};
				set["values."+(moment().month()+1)] = value;
	...
	seoPrerender: function(redisClient) {
		var renderPhatom = function(escaped_fragment, res) {
			phantom.create(function(err, ph) {
				if(err)
					console.log(err);
				return ph.createPage(function(err, page) {
					return page.open("http://"+ process.env.OPENSHIFT_APP_DNS + escaped_fragment, function(status) {
						return page.evaluate((function() {
							return document.getElementsByTagName('html')[0].innerHTML;
						}),function(err, result) {
							if(redisClient) {
								redisClient.set("seo_"+ escaped_fragment, result);
								redisClient.expire("seo_"+ escaped_fragment, 86400); // Cache for a day
							}
		...
		return function(req, res, next) {
			if(typeof(req.query._escaped_fragment_) !== "undefined") {
				if(redisClient) {
					redisClient.get("seo_"+ req.query._escaped_fragment_, function(err, reply) {
						if(reply) {
							return res.send(reply);
						} else {
							renderPhatom(req.query._escaped_fragment_, res);
						}
					});
				} else {
					renderPhatom(req.query._escaped_fragment_);
				}
			} else {
				next();
			}
