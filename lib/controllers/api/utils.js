...
module.exports = {
	...
	getModel: function(model, baseQuery, one) {
		var me = this;
		...
		return function(req, res) {
			...
			if(req.query.count != undefined) {
				...
				if(one) {
					targetQuery = Model.findOne(query);
				} else {
					targetQuery = Model.find(query);
				}
			}

			me.buildQuery(targetQuery, req.query).exec(function(err, items) {
				if(err) {
