...
module.exports = function (options, cb) {
  options = options || {};
  var month = options.month > 0 ? options.month - 1 : moment().month();
  var year = options.year || moment().year();

  console.log('Asked to fetch events starting with month="' + month + '" year="' + year + '".');
  ...
  console.log('Fetching events: start: "' + moment.unix(firstDayOfMonth).format('MMM DD YYYY') +
  ...
  async.series([
    ...
    function (callback) {
      console.log('Fetching tags for events...');
      ...
      devsite.fetchTags(function (err, tagsObject) {
        ...
        async.each(tags, function (tag, tagCallback) {
          ...
          console.log('Done fetching tags.');
          callback(err, 'two');
        });
      });
    }
  ], function (err) {
    console.log('Fetch events task complete.');
    if (err) {
      console.log(err);
    }
    // All done
