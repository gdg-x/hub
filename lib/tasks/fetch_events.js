...
module.exports = function(id, params, cb){
		...
		var lastDayOfMonth = moment().year(year).month(month+1).add('months', 1).date(10).seconds(0).minutes(0).hours(0).unix();
		var firstDayOfMonth = moment().year(year).month(month+1).date(1).subtract('days', 10).seconds(0).minutes(0).hours(0).unix();
