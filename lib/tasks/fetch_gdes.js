		...
		Gde.find({}).exec(function(err, oldGdeList) {
			...
			async.eachSeries(gdeList, function(gde, gdeCallback) {
				...
				if(match) {
					...
					plusQueue.push("https://www.googleapis.com/plus/v1/people/"+gplusId+"?fields=id&key="+config.keys.google.simpleApiKey, gde, function(err, res, gde) {
						...
						if(ngde.products.indexOf(gde.product) == -1) {
							...
							ngde.product_codes.push(slug(gde.product.toLowerCase().replace("+","plus").replace(":", "-").replace("--", "-")));
						}
