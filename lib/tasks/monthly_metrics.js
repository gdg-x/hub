...
module.exports = function(id, params, cb){
	async.series([
		function(callback) {
			...
			Chapter.count({},
				function(err, count) {
					utils.recordMonthlyMetric("gloal", "Global", "chapterCount", count);
		...
		function(callback) {
			DailyMetric.find({type: "plusOneCount", year: moment().year(), month: moment().subtract('months', 1).month()},
		...
		function(callback) {
			DailyMetric.find({type: "circledByCount", year: moment().year(), month: moment().subtract('months', 1).month()},
				function(err, metrics) {
					async.each(metrics,
						function(metric, metricCallback) {
		...
		function(callback) {
			Event.count({
				start: { '$gt': moment().subtract('months', 1).startOf('month').toDate() },
				end: { '$lt': moment().subtract('months', 1).endOf("month").toDate() } },
				function(err, count) {
		...
		function(callback) {
			Event.aggregate([
			    { $match: { start: { '$gte': moment().subtract('months', 1).startOf('month').toDate() },
			    			end: { '$lte': moment().subtract('months', 1).endOf("month").toDate() } } }
			  ...
			  , { $group: { /* execute 'grouping' */
				...
				async.each(items,
					function(country, countryCallback) {
						utils.recordMonthlyMetric(country._id, "Country", "chapterCount", country.chapters.length > 0 ? country.chapters.length  : 0);
						...
						Event.count({
							...
							function(err, count) {
								...
								Event.aggregate([
								    { $match: { chapter: { $in: country.chapters },
								    			start: { '$gte': moment().subtract('months', 1).startOf('month').toDate() },
								    			end: { '$lte': moment().subtract('months', 1).endOf("month").toDate() } } }
								  , { $group: { /* execute 'grouping' */
								          _id: 'participants' /* using the 'token' value as the _id */
								        , count: { $sum: '$participants' } /* create a sum value */
									...
									utils.recordMonthlyMetric(country._id, "Country", "participantCount", items.length > 0 ? items[0].count : 0);
									countryCallback(err);
								});
							}
						);
