...
module.exports = (function () {
    ...
    if (readyCount === 2) {
      ...
      self.on('group:taskdone', function (data) {
        if (tasks[data.id] && tasks[data.id].executor === data.sender) {
          // Task got executed
          taskDone(data.id, data.type, tasks[data.id].started, tasks[data.id].executor, data.err,
            data.result, tasks[data.id].cb);
        }
      });
  ...
  self = {
    connect: function (options) {
      ...
      emitter = redis.createClient(port, host).on('error', function (err) {
        console.error('ERR:REDIS: ' + err);
      });
      receiver = redis.createClient(port, host).on('error', function (err) {
        console.error('ERR:REDIS: ' + err);
      });
      ...
      if (auth) {
        console.log('Authorizing with Redis...');
        emitter.auth(auth);
        receiver.auth(auth);
        console.log('Redis Auth complete.');
      }
