...
module.exports = function() {
	...
	var ready = function() {
		...
		if(readyCount == 2) {
			...
			self.on('group:taskrequest', function(data) {
				if(!master && taskHandler[data.type]) {
	...
	self = {
		...
		sendTask: function(taskType, params, cb) {
			cb = cb || function() {};
			params = params || {};
			
			var id = uuid.v4();
			if(master && Object.keys(cluster).length > 0) {
				...
				taskHandler[taskType](id, params, function(err, result) {
					taskDone(id, taskType, moment().valueOf(), myId, err, result, cb);
				});
			} else {
